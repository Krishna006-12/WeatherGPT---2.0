...
export const getLiveEventsInputSchema = z.object({
  ...
  status: z.enum(["active", "monitoring", "resolved"]).optional().default("active"),
...
export class GetLiveEventsTool implements WeatherIntelligenceTool<GetLiveEventsInput, WeatherEvent[]> {
  ...
  async execute(input: GetLiveEventsInput): Promise<Result<WeatherEvent[]>> {
    ...
    try {
      const { keyword, locationName, status, limit } = parsed.data;
      const allEvents = await this.eventRepository.findAll({ status, limit: 50 });
      ...
      const matched = allEvents.filter((ev) => {
        ...
        const matchesLoc = locLower ? text.includes(locLower) || ev.location.name.toLowerCase().includes(locLower) || ev.location.country.toLowerCase().includes(locLower) : false;
